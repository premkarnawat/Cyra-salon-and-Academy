...
const FALLBACK: Package[] = [
  {
    ...
    features: ["Hair Spa + Scalp Massage", "Basic Cut & Blow Dry", "Eyebrow Threading", "Face Clean-Up"],
    is_active: true,
    sort_order: 1,
    created_at: "",
  },
  {
    ...
    features: ["Keratin / Smoothening", "Global Hair Colour", "Hair Spa Treatment", "Cut, Style & Blow Dry", "Face Bleach + Threading"],
    is_active: true,
    sort_order: 2,
    created_at: "",
  },
  {
    ...
    features: ["Pre-Bridal Facial", "Full Hair Colour + Treatment", "HD Bridal Makeup", "Saree / Lehenga Draping", "Photoshoot Styling", "2 Trial Sessions"],
    is_active: true,
    sort_order: 3,
    created_at: "",
  },
          ...
          {list.map((pkg) => {
            ...
            return (
              <StaggerItem key={pkg.id}>
                <div
                  className={`rounded-3xl overflow-hidden bg-[#F8F9FB] border transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_16px_48px_rgba(191,160,106,0.18)] ${
                    featured
                      ? "border-[rgba(191,160,106,0.4)] shadow-[0_4px_24px_rgba(191,160,106,0.1)]"
                      : "border-[rgba(191,160,106,0.15)]"
                  }`}
                >

                  {/* ✅ IMAGE FIX (perfect fit, no weird crop) */}
                  {pkg.image_url && (
                  ...
                  {/* CONTENT */}
                  <div className="p-6">
                    <div className="font-cormorant text-2xl text-[#1F2937] mb-3">
                      {pkg.name}
                    </div>

                    {/* Pricing */}
                    <div className="flex items-baseline gap-3 mb-4">
                      <span className="font-jost text-3xl font-black text-[#1F2937]">
                        {formatPrice(pkg.offer_price)}
                      </span>
                      <span className="font-jost text-sm line-through text-[#9CA3AF]">
