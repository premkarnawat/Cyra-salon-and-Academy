...
const FALLBACK: Package[] = [
  { id:"p1", name:"Glow", actual_price:2000, offer_price:1499, discount_percent:25, badge:"Starter", image_url:"https://images.unsplash.com/photo-1560066984-138dadb4c035?w=800&q=85", features:["Hair Spa + Scalp Massage"], is_active:true, sort_order:1, created_at:"" }
];
              ...
              <StaggerItem key={pkg.id}>
                <div className={`rounded-3xl overflow-hidden bg-[#F8F9FB] border transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_16px_48px_rgba(191,160,106,0.18)] flex flex-col h-full ${
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <div className="text-[1.6rem] mb-1">{pkg.name}</div>
                    ...
                    {pkg.free_offer_description && (
                      <p className="text-sm text-[var(--gold-dark)] mb-3">
                        🎁 {pkg.free_offer_description}
                      </p>
                    )}
                    ...
                    {/* CTA */}
                    <a
                      href={getWhatsAppLink(WHATSAPP_MESSAGES.booking(`${pkg.name}`))}
                      target="_blank"
                      className="bg-[#C9973A] text-white text-center py-3 rounded-xl"
                    >
