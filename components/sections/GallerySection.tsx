...
export function GallerySection({ gallery }: { gallery: GalleryItem[] }) {
  ...
  return (
    <section id="gallery" className="py-20 md:py-28 bg-[#F8F9FB]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <FadeIn>
          <SectionHeader
            tag="📸 Transformations"
            title={<>Our <em className="text-[var(--gold-light)] not-italic font-normal">Gallery</em></>}
            subtitle="Swipe or drag to explore · Auto-advances every 10 seconds"
          />
        </FadeIn>

        {/* ── Main carousel ── */}
        <div className="relative rounded-3xl overflow-hidden bg-white shadow-[0_16px_64px_rgba(0,0,0,0.1)] border border-[rgba(191,160,106,0.12)] select-none">
              ...
              >
                {item.media_type === "video" ? (
                  <div className="relative w-full h-full bg-black">
                    <video src={item.media_url} className="w-full h-full object-contain" muted playsInline controls controlsList="nodownload" />
                  </div>
                ) : (
                  <>
                    <Image
                ...
                {/* Title + counter bottom */}
                <div className="absolute bottom-0 left-0 right-0 px-6 pb-5 pt-10 pointer-events-none">
                  <div className="flex items-end justify-between">
                    <div>
                      {item.title && (
                        <p className="font-cormorant text-xl text-white drop-shadow mb-1">{item.title}</p>
                      )}
                    </div>
          ...
          {/* Progress bar — 10s timer */}
          <div className="h-[2px] bg-[rgba(191,160,106,0.15)]">
            <motion.div
              key={cur}
        ...
        {/* ── Dot indicators (tap to jump, NO arrows) ── */}
            ...
            <button
              ...
              className={`rounded-full transition-all duration-350 ${
                i === cur
                  ...
                  : "w-2.5 h-2.5 bg-[rgba(191,160,106,0.25)] hover:bg-[rgba(191,160,106,0.5)]"
              }`}
            />
          ))}
        </div>
        ...
        {/* Swipe hint */}
        <p className="text-center mt-3 font-jost text-[11px] text-[#9CA3AF]">
