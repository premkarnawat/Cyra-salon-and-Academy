...
export function Navbar({ config, onExploreOffers }: NavbarProps) {
  ...
  // Show ONLY the first word (e.g. "CYRA") — once, no duplication
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="h-16 flex items-center justify-between gap-3">
            ...
            {/* CENTER: Brand — shown ONCE only */}
            <button
              onClick={() => handleNavClick("#home")}
              className="flex flex-col items-center leading-none select-none"
              aria-label="Go home"
            >
              <span className="font-cinzel text-[1.3rem] sm:text-[1.5rem] tracking-[0.22em] text-[var(--gold-dark)] hover:text-[var(--gold)] transition-colors">
                {brandWord}
              </span>
              <span className="font-marcellus text-[0.48rem] sm:text-[0.54rem] tracking-[0.42em] uppercase text-[var(--gold)] opacity-70 mt-[3px]">
                Salon &amp; Academy
              </span>
            </button>
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <>
            <motion.div
              key="backdrop"
              className="fixed inset-0 z-[990] bg-black/35"
            ...
            >
              ...
              <div className="relative px-7 pt-10 pb-7 border-b border-[rgba(191,160,106,0.1)]">
                ...
                <div className="font-cinzel text-[1.7rem] tracking-[0.2em] text-[var(--gold-dark)]">{brandWord}</div>
                <div className="font-marcellus text-[0.58rem] tracking-[0.42em] uppercase text-[var(--gold)] opacity-70 mt-1.5">Salon &amp; Academy</div>
              ...
              <nav className="flex-1 py-3 overflow-y-auto" aria-label="Site navigation">
                ...
                {NAV_LINKS.map((link, i) => (
                  <motion.button key={link.href} onClick={() => handleNavClick(link.href)}
                    className="w-full text-left flex items-center gap-4 px-7 py-[13px] font-jost text-[13px] font-medium text-[#374151] hover:text-[var(--gold-dark)] hover:bg-[rgba(191,160,106,0.06)] border-l-[2.5px] border-transparent hover:border-[var(--gold)] transition-all"
                    initial={{ opacity:0, x:-16 }} animate={{ opacity:1, x:0 }}
                    transition={{ delay:i*0.045, duration:0.32 }}>
                    <span className="w-5 text-center text-[14px]" aria-hidden="true">{NAV_ICONS[link.label]??"✦"}</span>
              ...
              <div className="px-7 py-6 border-t border-[rgba(191,160,106,0.1)] bg-[rgba(191,160,106,0.025)]">
                <div className="space-y-2.5 mb-5">
                  {config.phone && (
                    <a href={`tel:${config.phone}`} className="flex items-center gap-2.5 text-[12px] text-[#6B7280] hover:text-[var(--gold-dark)] transition-colors">
                      <Phone size={12} className="text-[var(--gold)] flex-shrink-0"/><span className="font-medium">{config.phone}</span>
                    </a>
                  )}
                  {config.opening_hours && (
                    <div className="flex items-center gap-2.5 text-[12px] text-[#6B7280]">
                      <Clock size={12} className="text-[var(--gold)] flex-shrink-0"/><span>{config.opening_hours}</span>
                    </div>
                  )}
                  {config.address && (
                    <div className="flex items-start gap-2.5 text-[12px] text-[#6B7280]">
                      <MapPin size={12} className="text-[var(--gold)] flex-shrink-0 mt-0.5"/><span className="leading-relaxed">{config.address}</span>
                    </div>
                  )}
                </div>
                <a href={getWhatsAppLink(WHATSAPP_MESSAGES.general)} target="_blank" rel="noreferrer"
                  className="flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#1eb956] text-white rounded-xl px-4 py-3 text-[12px] font-semibold transition-colors">
