...
const FALLBACK_BANNERS: Banner[] = [
  ...
  },
  {
    ...
    subtitle: "Silky Smooth · Lasts 4–6 Months",
...
// ─── Content animation (re-runs on each slide change) ────────────────────────
...
export function HeroBanner({ banners, onExploreCTA }: HeroBannerProps) {
  ...
  const startTimer = useCallback(() => {
    ...
    setRunning(false);
    // tiny delay so progress bar resets visually
  ...
  function handleCTA(banner: Banner) {
    onExploreCTA?.();
    const target = banner.cta_link || "#offers";
    setTimeout(() => {
  ...
  return (
    <section
      id="home"
      className="relative h-[100svh] min-h-[600px] max-h-[1000px] overflow-hidden bg-black"
      aria-label="Hero banner"
    >
      {/* ── SLIDE (AnimatePresence swaps full slide) ── */}
      ...
      {/* ── CONTENT (re-animates every slide change) ── */}
        ...
        >
          ...
          <motion.div
            ...
            className="
              ...
              border border-[rgba(191,160,106,0.4)]
              bg-[rgba(191,160,106,0.12)]
              backdrop-blur-sm
            "
          >
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--gold)] animate-pulse" />
            <span className="font-jost text-[10.5px] tracking-[0.28em] uppercase text-[var(--gold-light)]">
          ...
          <motion.h1
            variants={contentVariants}
            className="
              font-cormorant font-bold leading-[1.0]
          ...
          {/* ── Discount badge — RED highlight as per spec ── */}
          {banner.discount_text && (
            ...
            >
              ...
              />
              {/* Red discount text */}
              <span className="relative text-[#FF4444] drop-shadow-[0_2px_24px_rgba(192,57,43,0.6)]">
                {banner.discount_text}
              </span>
            </motion.div>
          )}
          ...
          <motion.div
            variants={contentVariants}
            className="w-16 h-px bg-gradient-to-r from-transparent via-[var(--gold)] to-transparent my-4 sm:my-5"
          />
          ...
          <motion.button
            ...
            className="
              ...
              tracking-[0.28em]
              shadow-[0_12px_40px_rgba(191,160,106,0.35)]
              hover:shadow-[0_18px_50px_rgba(191,160,106,0.45)]
          ...
          <motion.a
            ...
            className="
              mt-5
              font-jost text-[11px] tracking-[0.15em]
              text-white/35
      ...
      <button
        ...
        className="
          ...
          rounded-full
          bg-black/30 hover:bg-[var(--gold)]/85
          border border-white/20 hover:border-[var(--gold)]
        ...
        {list.map((_, i) => (
          <button
            ...
            className={`
              ...
              ${i === current
                ? "w-7 h-[7px] bg-[var(--gold)]"
                : "w-[7px] h-[7px] bg-white/35 hover:bg-white/60"
              }
            `}
          />
        ))}
      ...
      <div className="absolute top-5 right-5 z-20 flex items-center gap-1.5">
        <span className="font-jost text-white font-semibold text-sm leading-none">
          {String(current + 1).padStart(2, "0")}
        </span>
        <span className="w-5 h-px bg-white/30" />
        <span className="font-jost text-white/40 text-xs leading-none">
