...
export async function GET() {
  ...
  const { data, error } = await sb.from("gallery").select("*").eq("is_active",true).order("sort_order",{ascending:true});
...
export async function POST(req: Request) {
  ...
  const { data, error } = await SB().from("gallery").insert(body).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
