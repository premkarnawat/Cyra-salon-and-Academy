...
export async function GET() {
  ...
  const { data, error } = await sb.from("offers").select("*").eq("is_active",true).order("sort_order",{ascending:true});
...
export async function PUT(req: Request) {
  ...
  const { data, error } = await SB().from("offers").update(rest).eq("id",id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
